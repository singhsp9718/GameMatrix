...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn { flex: 1; min-width: 140px; text-align: center; padding: 11px; border-radius: 20px; font-weight: bold; font-size: 13px; text-decoration: none; transition: background 0.1s; cursor: pointer; }
        .btn-appstore { background-color: #00da77; color: #fff; border: none; }
        .btn-appstore:hover { background-color: #00be68; }
        .btn-googleplay { background-color: #00da77; color: #fff; border: none; }
        ...
        /* DYNAMIC MODAL BOX UI OVERLAY */
        .modal-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 9999; justify-content: center; align-items: center; opacity: 0; transition: opacity 0.2s ease; }
        .modal-box { background: #fff; max-width: 400px; width: 90%; padding: 24px; border-radius: 14px; text-align: left; box-shadow: 0 4px 20px rgba(0,0,0,0.15); transform: scale(0.9); transition: transform 0.2s ease; }
        .modal-overlay.active { display: flex; opacity: 1; }
        .modal-overlay.active .modal-box { transform: scale(1); }
        .modal-title { font-size: 16px; font-weight: bold; color: #202124; margin-bottom: 8px; }
        .modal-msg { font-size: 13.5px; color: #5f6368; line-height: 1.5; margin-bottom: 20px; }
        .modal-btn-row { display: flex; justify-content: flex-end; gap: 12px; }
        .m-btn { padding: 8px 16px; font-size: 13px; font-weight: bold; border-radius: 8px; cursor: pointer; border: none; }
        .m-btn-cancel { background: #f1f3f4; color: #3c4043; }
        .m-btn-cancel:hover { background: #e8eaed; }
        .m-btn-continue { background: #00da77; color: #fff; }
        .m-btn-continue:hover { background: #00be68; }
    ...
    <script>
        ...
        function triggerRedirectNotice(targetStoreUrl, storeName) {
            globalRedirectTargetUrl = targetStoreUrl;
            document.getElementById("modalStoreName").innerText = storeName;
            const modal = document.getElementById("redirectNoticeModal");
            modal.classList.add("active");
        }

        function dismissRedirectModal() {
            const modal = document.getElementById("redirectNoticeModal");
            modal.classList.remove("active");
            globalRedirectTargetUrl = "";
        }

        function executeFinalRedirect() {
            if(globalRedirectTargetUrl !== "") {
                window.open(globalRedirectTargetUrl, '_blank');
                dismissRedirectModal();
            }
        }
    </script>
</head>
<body>
    ...
    <div class="wrapper">
        <main class="main-content">
            ...
            <div class="btn-row">
                <button onclick="triggerRedirectNotice('${appleStoreUrl}', 'Apple App Store')" class="btn btn-appstore">App Store</button>
                <button onclick="triggerRedirectNotice('${playUrl}', 'Google Play Store')" class="btn btn-googleplay">Google Play</button>
            </div>
    ...
    <!-- HIGH PREMIUM DYNAMIC REDIRECTION DIALOG ENGINE -->
    <div id="redirectNoticeModal" class="modal-overlay" onclick="dismissRedirectModal()">
        <div class="modal-box" onclick="event.stopPropagation()">
            <div class="modal-title">Redirecting to Official Store</div>
            <div class="modal-msg">You are now leaving GameMatrix (FavNinja.com) and being safely redirected to the official <span id="modalStoreName" style="font-weight: bold; color: #00875a;">Store</span>. Are you sure you want to continue?</div>
            <div class="modal-btn-row">
                <button class="m-btn m-btn-cancel" onclick="dismissRedirectModal()">CANCEL</button>
                <button class="m-btn m-btn-continue" onclick="executeFinalRedirect()">CONTINUE</button>
            </div>
        </div>
    </div>
